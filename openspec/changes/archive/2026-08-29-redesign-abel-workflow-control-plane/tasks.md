...
- [x] 1.1 `T1-control-store-delivery` — replace caller-owned workflow mechanics with v2 control contracts, durable state primitives, and a canonical delivery compiler.
...
- [x] 2.1 `T2-route-policy-broker` — replace environment endpoint pinning with a visible, bounded, capability-aware route broker.
...
- [x] 3.1 `T3-workspace-revisions` — introduce content-addressed artifacts, immutable workspace manifests, and an isolation backend boundary.
...
- [x] 4.1 `T4-transactional-apply` — move all candidate and cumulative verification into immutable revisions and make final delivery recoverable.
...
- [x] 5.1 `T5-worker-artifacts-ledger` — replace one-shot diff retention and conversational continuity with sealed segments and a durable task ledger.
...
- [x] 6.1 `T6-workflow-engine` — make `WorkflowEngine` the sole v2 transition authority and integrate durable scheduling, recovery, cancellation, and lifecycle control behind the active bootstrap selector.
  - Objective: admit only the schema-v3 canonical plan and receipt inside the v2 engine, derive readiness and operations there, queue conflicts durably, enforce the sealed per-phase/per-operation artifact-correction attempt bound, drive route-bound ephemeral attempts, commit verified revision deltas, recover leases/apply including pending cancel/discard, expose local status and typed `rebind`, and prepare a resumable bootstrap handoff while keeping the exact v1 bootstrap activation selected through T7 acceptance.
  ...
  - Requirement/Scenario ownership:
    ...
    - `specs/abel-workflow-prompt-package/spec.md#Implementation behavior/Ordinary Implement failure occurs`
    - `specs/abel-workflow-prompt-package/spec.md#Implementation behavior/Repair budget is exhausted`
...
- [x] 7.1 `T7-workflow-ux-distribution` — project durable semantic state truthfully, synchronize the v2 workflow/package contract, and close the one-time bootstrap safely.
  - Objective: update activity rendering, all four workflow prompts, shared Skill, professional Agents, docs, package metadata, seed/pack checks, and provenance so explicit invocation is the only activation signal, stage responsibilities remain disjoint, ordinary failure recovery stays inside Implement, and only an actually completed run receives a success checkmark; keep v1 selected while the complete acceptance matrix runs, then durably commit `BootstrapHandoff` before the one-way v2 selector cutover.
  ...
  - Requirement/Scenario ownership:
    ...
    - `specs/abel-workflow-prompt-package/spec.md#Explicit invocation and stage isolation/Ordinary engineering work mentions Abel`
    - `specs/abel-workflow-prompt-package/spec.md#Explicit invocation and stage isolation/Verified workflow prompt is invoked`
    - `specs/abel-workflow-prompt-package/spec.md#Explicit invocation and stage isolation/Init is invoked`
    - `specs/abel-workflow-prompt-package/spec.md#Initialization behavior/Existing project is initialized twice`
    - `specs/abel-workflow-prompt-package/spec.md#Initialization behavior/Optional research Skill is missing`
    - `specs/abel-workflow-prompt-package/spec.md#Initialization behavior/Core workflow skill is unavailable`
    - `specs/abel-workflow-prompt-package/spec.md#Initialization behavior/Research skills are available`
    - `specs/abel-workflow-prompt-package/spec.md#Initialization behavior/Research skill is missing`
    - `specs/abel-workflow-prompt-package/spec.md#Initialization behavior/Non-init skills are absent`
    - `specs/abel-workflow-prompt-package/spec.md#Initialization behavior/Default project path`
    - `specs/abel-workflow-prompt-package/spec.md#Initialization behavior/OpenSpec is absent with Bun available`
    - `specs/abel-workflow-prompt-package/spec.md#Initialization behavior/Bun is unavailable and npm is available`
    - `specs/abel-workflow-prompt-package/spec.md#Initialization behavior/OpenSpec installation fails`
    - `specs/abel-workflow-prompt-package/spec.md#Initialization behavior/Existing AGENTS content is present`
    ...
    - `specs/abel-workflow-prompt-package/spec.md#Diagnosis behavior/Existing defect is repaired`
    - `specs/abel-workflow-prompt-package/spec.md#Diagnosis behavior/Root cause is not proven`
    - `specs/abel-workflow-prompt-package/spec.md#Diagnosis behavior/Diagnosis requires new product scope`
    - `specs/abel-workflow-prompt-package/spec.md#Diagnosis behavior/Root cause is unverified`
    - `specs/abel-workflow-prompt-package/spec.md#Diagnosis behavior/Regression-first repair`
    - `specs/abel-workflow-prompt-package/spec.md#Diagnosis behavior/Parent confirms regression Red`
    - `specs/abel-workflow-prompt-package/spec.md#Diagnosis behavior/Requested fix changes behavior`
  ...
  - Dispatch context: accepted T6 output and full change workspace; design decisions 4, 9, 11, and 13 plus the migration cutover; `src/runtime.ts`, `src/worker.ts`, `src/scheduler.ts`, `src/drain.ts`, `src/activation.ts`, `src/index.ts`, `src/subagent-activity.ts`; all four workflow prompts; shared Skill; all four package Agents; README/package/distribution/provenance files; and all UX/package/activation tests below.
  - Red phase: read `src/runtime.ts`, `src/worker.ts`, `src/scheduler.ts`, `src/drain.ts`, `src/activation.ts`, `src/index.ts`, `src/subagent-activity.ts`, `prompts/abel-init.md`, `prompts/abel-design.md`, `prompts/abel-diagnose.md`, `prompts/abel-implement.md`, `skills/abel-workflow/SKILL.md`, `agents/design-explorer.md`, `agents/implementation-worker.md`, `agents/diagnosis-worker.md`, `agents/contract-reviewer.md`, `README.md`, `biome.json`, `package.json`, `scripts/check-agents.mjs`, `scripts/pack-check.mjs`, `scripts/seed-acceptance.mjs`, `scripts/traceability-check.mjs`, `provenance/adapted-modules.yaml`, `config/routes.example.json`, `test/workflow-engine.integration.test.ts`, `test/runtime-worker-broker.integration.test.ts`, `test/runtime-recovery.property.test.ts`, `test/runtime-activity.property.test.ts`, `test/activation.test.ts`, `test/prompt-activation.integration.test.ts`, `test/subagent-activity.property.test.ts`, `test/subagent-activity.integration.test.ts`, `test/prompts.test.mjs`, `test/stage-contracts.test.mjs`, `test/workflow-skill.test.mjs`, `test/distribution.test.mjs`, `test/package-contract.test.mjs`, `test/independence.test.ts`, and `test/traceability-check.test.mjs`; write only new `test/workflow-ux.integration.test.ts`. Verification id `T7-workflow-ux-distribution-red`; expected failure `[CADENCE-V2:T7-workflow-ux-distribution]` because prompts/package/UI still describe v1 graph attempts, ordinary failures can be misrouted between stages, implicit Skill discovery can be mistaken for invocation, the package lacks an acceptance-gated resumable cutover, and tool settlement is conflated with completion.
  - Green phase: read the Red scope plus `test/workflow-ux.integration.test.ts`; modify `src/runtime.ts`, `src/worker.ts`, `src/scheduler.ts`, `src/drain.ts`, `src/activation.ts`, `src/index.ts`, `src/subagent-activity.ts`, `prompts/abel-init.md`, `prompts/abel-design.md`, `prompts/abel-diagnose.md`, `prompts/abel-implement.md`, `skills/abel-workflow/SKILL.md`, `agents/design-explorer.md`, `agents/implementation-worker.md`, `agents/diagnosis-worker.md`, `agents/contract-reviewer.md`, `README.md`, `biome.json`, `package.json`, `scripts/check-agents.mjs`, `scripts/pack-check.mjs`, `scripts/seed-acceptance.mjs`, `scripts/traceability-check.mjs`, `provenance/adapted-modules.yaml`, `test/runtime-worker-broker.integration.test.ts`, `test/activation.test.ts`, `test/prompt-activation.integration.test.ts`, `test/subagent-activity.property.test.ts`, `test/subagent-activity.integration.test.ts`, `test/prompts.test.mjs`, `test/stage-contracts.test.mjs`, `test/workflow-skill.test.mjs`, `test/distribution.test.mjs`, `test/package-contract.test.mjs`, `test/independence.test.ts`, and `test/traceability-check.test.mjs`; preserve `config/routes.example.json`, the immutable WorkflowEngine Red test, the remaining T6 recovery/activity tests, and the T7 Red test unchanged. Verification id `T7-workflow-ux-distribution-green`; Green exposes explicit-only workflow activation, isolated Init/Design/Diagnose responsibilities, same-run ordinary Implement recovery, queued/connecting/waiting/running/validating/retrying/verifying/paused/approval-needed/applying/recovering/cancelled/discarded/rejected/completed truthfully, exposes `rebind`, keeps the exact v1 bootstrap selected through all acceptance checks, commits a reload-resumable v2 handoff before selector CAS, rejects every other v1 delivery after cutover, ships routes example and v2 resources, raises Node to `>=22.13.0`, and retains no hidden checkout/global deployment dependency.
