/* Windows x64 host-trusted Job supervisor, protocol v2. Wire: nine LE uint32s,
...
static void report(const char *outcome, const char *reason, int root, int settled, DWORD exitCode) {
  printf("{\"version\":2,\"outcome\":\"%s\",\"reason\":\"%s\",\"rootExited\":%s,\"managedSettled\":%s,\"exitCode\":%lu}\n",
    outcome, reason, root ? "true" : "false", settled ? "true" : "false", (unsigned long)exitCode);
  fflush(stdout);
}
int wmain(int argc, wchar_t **argv) {
  DWORD h[9], total = 0, i, active = 1, count, available, exitCode = 1;
  wchar_t *parts[4] = {0};
  ...
  BOOL inJob = FALSE;
  int probe = argc == 2 && wcscmp(argv[1], L"--probe") == 0;
  { HANDLE watchdog = CreateThread(NULL, 0, watchdogThread, NULL, 0, NULL);
    if (!watchdog) return 2;
    CloseHandle(watchdog);
  }
  if (argc > 1 && !probe) goto done;
  if (!exact(h, sizeof(h)) || h[0] != 2 || h[8] != 0 || h[1] < 1 || h[1] > 86400000 || h[2] < 1 || h[2] > 10000 || h[3] != 0) goto done;
  for (i = 0; i < 4; i++) {
  ...
  InitializeProcThreadAttributeList(NULL, 2, 0, &attributeBytes);
  si.lpAttributeList = (LPPROC_THREAD_ATTRIBUTE_LIST)malloc(attributeBytes);
  if (!si.lpAttributeList) goto done;
  if (!InitializeProcThreadAttributeList(si.lpAttributeList, 2, 0, &attributeBytes)) { free(si.lpAttributeList); si.lpAttributeList = NULL; goto done; }
  ...
  /* Atomic Job membership closes the helper-loss window between creation and
   * AssignProcessToJobObject. The root cannot exist outside this Job, even
   * while suspended; the Job handle is never inherited by target descendants. */
  if (!UpdateProcThreadAttribute(si.lpAttributeList, 0, PROC_THREAD_ATTRIBUTE_JOB_LIST, &job, sizeof(job), NULL, NULL)) goto done;
  ...
  if (!IsProcessInJob(pi.hProcess, job, &inJob) || !inJob) goto done;
  controlEvent = CreateEventW(NULL, TRUE, FALSE, NULL);
  if (!controlEvent) goto done;
  if (!probe) { HANDLE thread = CreateThread(NULL, 0, controlThread, NULL, 0, NULL);
    if (!thread) goto done;
    CloseHandle(thread);
  }
  ...
  if (root && !GetExitCodeProcess(pi.hProcess, &exitCode)) { settled = 0; reason = "termination-unconfirmed"; }
  report(settled ? (strcmp(reason, "exit") == 0 ? "complete" : "failed") : "uncertain", reason, root, settled, exitCode);
  result = settled ? 0 : 2;
  goto cleanup;
done:
  if (pi.hProcess) { TerminateProcess(pi.hProcess, 1); settled = WaitForSingleObject(pi.hProcess, 5000) == WAIT_OBJECT_0; }
  else settled = 1;
  report(settled ? "failed" : "uncertain", settled ? "launch-failed" : "termination-unconfirmed", pi.hProcess != NULL && settled, settled, 1);
  result = settled ? 0 : 2;
cleanup:
