...
// src/remove-path.ts
import { lstatSync as lstatSync2, readdirSync, rmdirSync, rmSync, unlinkSync as unlinkSync2 } from "node:fs";
import path2 from "node:path";
function removePathSync(target, options = {}) {
  ...
  if (stat.isDirectory() && !stat.isSymbolicLink()) {
    ...
    for (const name of readdirSync(target))
      removePathSync(path2.join(target, name), options);
    rmdirSync(target);
  } else
    unlinkSync2(target);
}
...
function prepareVerificationEnvironmentIo(root, dependencyOwner, runnerBindings, profile, environment, checkCancelled, privateRoot) {
  const cleanup = () => removePathSync(privateRoot, { recursive: true, force: true });
  try {
    ...
    for (const directory of [
      ...
      mkdirSync2(path3.join(privateRoot, directory), { mode: 448 });
    }
    if (native) {
      mkdirSync2(path3.join(privateRoot, "home/AppData/Roaming"), {
        recursive: true
      });
      mkdirSync2(path3.join(privateRoot, "home/AppData/Local"), {
        recursive: true
      });
    }
    writeFileSync2(path3.join(privateRoot, "home/user.npmrc"), "");
    writeFileSync2(path3.join(privateRoot, "home/global.npmrc"), "");
    const dependencies = path3.join(dependencyOwner, "node_modules");
    const stat = lstatSync3(dependencies, { throwIfNoEntry: false });
    if (stat && !native) {
      const target = path3.join(root, "node_modules");
      const targetStat = lstatSync3(target, { throwIfNoEntry: false });
      ...
      const view = path3.join(privateRoot, "dependencies");
      mounts.push({ source: view, target: "/workspace/node_modules" });
      const entries = readdirSync2(dependencies);
      ...
      const projectEntry = (name) => {
        ...
        if (CACHES.includes(name))
          return;
        const source = path3.join(dependencies, name);
        const placeholder = path3.join(view, name);
        const entry = lstatSync3(source);
        if (entry.isSymbolicLink()) {
          const resolved = realpathSync(source);
          if (within(canonical, resolved)) {
            symlinkSync(path3.relative(path3.dirname(source), resolved), placeholder);
          } else if (within(realpathSync(dependencyOwner), resolved)) {
            const relative = path3.relative(realpathSync(dependencyOwner), resolved);
            const candidate = path3.join(root, relative);
            if (!lstatSync3(candidate, { throwIfNoEntry: false }))
              throw new Error("workspace-dependency-missing");
            symlinkSync(local ? candidate : `/workspace/${relative.split(path3.sep).join("/")}`, placeholder);
          } else
            throw new Error("dependency-path-unsafe");
        } else if (entry.isDirectory() && name.startsWith("@") && !name.includes(path3.sep)) {
          mkdirSync2(placeholder);
          for (const child of readdirSync2(source))
            projectEntry(path3.join(name, child));
        } else {
          ...
          mounts.push({
            source,
            target: `/workspace/node_modules/${name.split(path3.sep).join("/")}`
          });
        }
      };
      ...
      for (const cache of CACHES) {
        mkdirSync2(path3.join(view, cache));
        const source = path3.join(privateRoot, "cache", cache);
        mkdirSync2(source);
        mounts.push({
          source,
    ...
    if (local && stat && !native) {
      const copyEntry = (name) => {
        checkCancelled();
        const source = path3.join(dependencies, name);
        const destination = path3.join(root, "node_modules", name);
        if (CACHES.includes(name)) {
        ...
        if (stat2.isSymbolicLink()) {
          const resolved = realpathSync(source);
          const translated = within(canonicalRoot(dependencyOwner), resolved) ? path3.join(root, path3.relative(canonicalRoot(dependencyOwner), resolved)) : resolved;
          symlinkSync(translated, destination);
        } else if (stat2.isDirectory() && name.startsWith("@") && !name.includes(path3.sep)) {
          mkdirSync2(destination);
          for (const child of readdirSync2(source))
            copyEntry(path3.join(name, child));
        } else
      ...
      for (const name of readdirSync2(path3.join(privateRoot, "dependencies")))
        copyEntry(name);
    }
    if (native && stat) {
      const target = path3.join(root, "node_modules");
      ...
      const copy = (source, destination) => {
        ...
        if (entry.isSymbolicLink()) {
          ...
          if (!within(owner, resolved))
            throw new Error("dependency-path-unsafe");
          const candidate = path3.join(root, path3.relative(owner, resolved));
          if (!within(realpathSync(dependencies), resolved) && !lstatSync3(candidate, { throwIfNoEntry: false }))
            throw new Error("workspace-dependency-missing");
          const linked = lstatSync3(resolved);
          ...
          for (const name of readdirSync2(source)) {
            if (source === dependencies && CACHES.includes(name)) {
              mkdirSync2(path3.join(destination, name));
              continue;
            }
            copy(path3.join(source, name), path3.join(destination, name));
          }
        } else if (entry.isFile())
    ...
    const bindings = runnerBindings.map((binding) => {
      if (native) {
        const translate2 = (value) => path3.isAbsolute(value) && within(dependencyOwner, value) ? path3.join(root, path3.relative(dependencyOwner, value)) : value;
        return {
          ...binding,
      ...
      if (local || !binding.mountSource)
        return { ...binding };
      const source = path3.resolve(binding.mountSource);
      const stat2 = lstatSync3(source);
      ...
      const translate = (file) => path3.posix.join(target, ...path3.relative(source, file).split(path3.sep));
      return {
        ...binding,
        executablePath: translate(lstatSync3(binding.executablePath).isSymbolicLink() && !path3.isAbsolute(readlinkSync(binding.executablePath)) ? binding.executablePath : executable),
        ...binding.fixedArgs ? {
          fixedArgs: binding.fixedArgs.map((arg) => path3.isAbsolute(arg) && within(source, arg) ? translate(arg) : arg)
        } : {}
      };
    });
    checkCancelled();
    return {
      ...
      environment: {
        ...
        ...native ? {
          USERPROFILE: path3.join(privateRoot, "home"),
          APPDATA: path3.join(privateRoot, "home", "AppData", "Roaming"),
          LOCALAPPDATA: path3.join(privateRoot, "home", "AppData", "Local"),
          TEMP: path3.join(privateRoot, "tmp"),
          TMP: path3.join(privateRoot, "tmp")
        } : {},
        CI: "1",
        HOME: local ? path3.join(privateRoot, "home") : "/cadence/home",
        XDG_CACHE_HOME: local ? path3.join(privateRoot, "cache") : "/cadence/cache",
        TMPDIR: local ? path3.join(privateRoot, "tmp") : "/cadence/tmp",
        npm_config_cache: local ? path3.join(privateRoot, "cache/npm") : "/cadence/cache/npm",
        npm_config_userconfig: local ? path3.join(privateRoot, "home/user.npmrc") : "/cadence/home/user.npmrc",
        npm_config_globalconfig: local ? path3.join(privateRoot, "home/global.npmrc") : "/cadence/home/global.npmrc",
        PATH: [
          ...new Set([
            ...bindings.filter((binding) => binding.mountSource),
            ...bindings.filter((binding) => !binding.mountSource)
          ].map((binding) => (local ? path3 : path3.posix).dirname(binding.executablePath))),
          ...local ? (environment.PATH ?? "").split(path3.delimiter) : [],
          ...native ? [] : ["/usr/local/bin", "/usr/bin", "/bin"]
        ].join(local ? path3.delimiter : ":")
      }
    };
  } catch (error) {
    cleanup();
...
function removeEntry(file, checkCancelled) {
  ...
  if (!stat)
    return;
  if (stat.isDirectory() && !stat.isSymbolicLink())
    for (const name of readdirSync2(file))
      removeEntry(path3.join(file, name), checkCancelled);
  removePathSync(file, {
...
function verificationEnvironmentDigest(roots, checkCancelled = () => {}) {
  ...
  const visit = (file, relative, dependencyRoot) => {
    ...
    if (stat.isDirectory()) {
      ...
      for (const name of readdirSync3(file).sort()) {
        if (dependencyRoot && [".vite", ".vite-temp"].includes(name))
          continue;
        visit(path4.join(file, name), `${relative}/${name}`, false);
      }
      return;
    }
    if (!stat.isFile())
  ...
  for (const root of [...new Set(roots)].sort())
    visit(root, root, path4.basename(root) === "node_modules");
...
function observeSafePath(root, relative) {
  ...
  try {
    ...
    const rootStat = lstatSync5(resolvedRoot, { throwIfNoEntry: false });
    ...
    for (const [index, part] of parts.entries()) {
      current = path5.join(current, part);
      if (!current.startsWith(`${resolvedRoot}${path5.sep}`)) {
        return { kind: "unsafe", reason: "invalid-path" };
      }
      const stat = lstatSync5(current, { throwIfNoEntry: false });
      if (!stat)
        return { kind: "absent" };
      if (stat.isSymbolicLink()) {
...
function ensureDirectory2(directory) {
  const stat = lstatSync6(directory, { throwIfNoEntry: false });
  if (stat) {
    if (stat.isSymbolicLink() || !stat.isDirectory()) {
  ...
  let cursor = path6.dirname(directory);
  while (cursor !== path6.dirname(cursor)) {
    const ancestor = lstatSync6(cursor, { throwIfNoEntry: false });
    if (ancestor) {
      if (ancestor.isSymbolicLink() || !ancestor.isDirectory()) {
        throw new Error("unsafe-workspace-path");
      }
  ...
  const created = lstatSync6(directory);
  if (created.isSymbolicLink() || !created.isDirectory()) {
    throw new Error("unsafe-workspace-path");
  }
  chmodSync2(directory, 448);
...
function writeAtomic2(target, bytes) {
  ...
  const temporary = path6.join(directory, `.${path6.basename(target)}.${process.pid}.${randomUUID2()}.tmp`);
...
class WorkspaceStore {
  ...
  #revisionPath(id) {
    ...
    return path6.join(this.#revisions, id.slice(0, 2), `${id}.json`);
  }
  #storeRevision(parentRevisionId, rawEntries) {
    ...
    const existing = lstatSync6(target, { throwIfNoEntry: false });
    if (existing) {
...
`));
  ...
  captureBaseline(input) {
    const consumerRoot = path6.resolve(input.consumerRoot);
    const rootStat = lstatSync6(consumerRoot, { throwIfNoEntry: false });
    if (!rootStat?.isDirectory() || rootStat.isSymbolicLink()) {
      throw new Error("unsafe-workspace-path");
    }
  ...
  #captureBaselineFiles(input, tracked, consumerRoot, excluded) {
    ...
    for (const relative of selected) {
      ...
      if (observation.kind !== "file")
        throw new Error("unsafe-workspace-path");
      const absolute = path6.join(consumerRoot, ...relative.split("/"));
  ...
  getRevision(id) {
    const target = this.#revisionPath(id);
    const stat = lstatSync6(target, { throwIfNoEntry: false });
    if (!stat?.isFile() || stat.isSymbolicLink()) {
  ...
  materialize(revisionIdValue, destination) {
    ...
    const root = path6.resolve(destination);
    ...
    for (const [relative, entry] of Object.entries(revision.entries)) {
      ...
      for (const segment of segments.slice(0, -1)) {
        parent = path6.join(parent, segment);
        ensureDirectory2(parent);
      }
      const target = path6.join(root, ...segments);
      if (observation.kind !== "absent") {
        throw new Error("workspace-materialize-conflict");
      }
  ...
  async captureBaselineWithGit(input, signal) {
    ...
    const consumerRoot = path6.resolve(input.consumerRoot);
    const stat = lstatSync6(consumerRoot, { throwIfNoEntry: false });
    if (!stat?.isDirectory() || stat.isSymbolicLink())
      throw new Error("unsafe-workspace-path");
    const excluded = [".git", "node_modules", ...input.excludedPaths ?? []];
    for (const relative of excluded)
