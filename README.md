...
每次扩展启动、重载或恢复会话，Cadence 都会在输入框上方显示一张简短研究能力提示卡，不需要额外记住配置或诊断命令。
卡片只检查本地配置，显示 Context7、Grok 与可选 Tavily 的配置状态及当前生效文件；“已配置”不代表网络、认证或服务能力已验证。
开始处理任务后卡片收起，下次加载重新显示；不记录“已看过”或“暂时不用”来抑制后续提示。
安装 Cadence 时会创建用户级模板 `~/.pi/agent/cadence/.env`（已有文件不会覆盖）；卡片本身不会联网、调用模型、写入配置、索要密钥或自动激活 Abel 工作流，可选服务未配置不阻塞普通任务。
未信任的项目只显示未检查提示，不读取研究配置。

安装后用户级模板已包含 Grok 配置占位字段；Context7 无配置文件时仍使用默认地址匿名访问，显式配置文件损坏仍会报错。
联网搜索需要在本地文件中配置以下内容，每项一行（替换为实际服务地址和密钥）：
...
安装器会创建用户级 `~/.pi/agent/cadence/.env` 及其父目录，并且只在文件不存在时写入模板；也可手动创建该文件以跨项目复用。
如果项目已有 `.pi/cadence/.env`，它按整文件优先，**不会与用户级文件合并**；请编辑卡片显示的生效文件，不要覆盖已有无关配置。
研究 Skills 不读取 shell 中的 API 环境变量，也不读取安装包旁的 `.env`；保存后下次 Skill 调用生效，无需重启。
可选 `CONTEXT7_API_KEY` 提供认证访问；可选 `TAVILY_API_KEY` 启用扩展检索、抓取和站点映射，`TAVILY_ENABLED=false` 可关闭它。
完整字段见 [配置模板](config/.env.example)，配置文件限普通文件且不超过 64 KiB。
密钥只在本地编辑，勿发送到聊天或提交 Git；使用项目级文件时应将其加入忽略规则。

TUI 显示卡片；RPC 通过非阻塞 `setWidget` 事件发送同一卡片，Web UI 需实现该事件的展示。
Print / JSON 无交互界面，不额外输出卡片、不等待输入，也不污染标准输出；可按上述路径直接配置。
本版本提供提示与本地编辑指引，不提供密钥输入向导或自动重试搜索。
...
任务 `baselineVerification` 只读取 run 的原始不可变快照；只有 affected 输入均为原始安全普通文件时，编译器才允许省略并推导它。
新增 Red 测试仍完整参与阶段、affected、累计和最终验收。
独立任务共享四个执行槽位，单个任务结算后立即补位；其他 run 释放容量也会唤醒排队工作。
局部 baseline 或能力缺失只阻塞相关任务及依赖者，完整验收和事务 apply 保留全局屏障。
Design 的 `summary.parallelism` 展示最多四项互不冲突的初始任务组、明确标为后续兼容关系的任务对、producer 等待、串行原因和全局验证屏障。
初始组假设共享槽位可用；实际运行仍检查容量、能力和 currentness。
`summary.implementation` 同时展示已接受的验收/限制 ID、技术修订可用的写入范围、依赖与验证模式，让 Design 能检查后续修复是否有足够权限；摘要本身不授予权限，也不宣称运行环境已就绪。
Design 集中解决用户决策和已知的外部前提，按受影响模块及必要集成修复确定政策范围，按精确文件划分初始任务。
Implement 以已接受目标为持续执行目标：父代理负责范围内的推荐决策、拆分、修复、重新编译和继续执行；调度器持续填充四个共享槽位。
父代理在仍有可执行后续动作时提前结束回合，宿主会重新读取同一 change 的本地状态，并接续同阶段工作，不需要用户再次发送“继续”。
取消、退出、模型中断、待处理用户消息和无进展保护会阻止自动接续；接续不扩大授权、不重置恢复额度，也不将缺少凭据或服务的外部阻塞伪报为完成。
进程中断后的 run 在用户再次显式调用 Implement 时提供同一 run 的恢复动作；已有 apply journal 的结算不消耗 Worker 额度，仍由引擎控制。
...
父控制面从原始快照证明的 baseline 输入缺陷和任务过大可以进入该修订通道，不必伪装成用户决策。
单独的错误码、hash/receipt 异常、不安全路径和未知错误不授予修订权限。
相同 baseline 合同、输入和环境下的失败会持久保留，重复 resume 不再执行相同失败检查或启动 Worker；可观测前提变化后才在原预算内恢复。
baseline 专属修订保留兼容的已提交阶段、任务成果与原 run。
...
`continuation.kind: "inspect-recovery"` 将已知故障交给父代理调查并选择具体修复策略；它是指导信息，不是新的 dispatch action。
纠错耗尽时，`conditionalCommands` 明确给出需要原样携带恢复凭据的 resume 条件；有证据支持的计划调整仍需经过同一 change 的编译和验证。
